/**
 * Official Sarh promote catalog — billing source of truth.
 *
 * Featured / pin / visibility amounts are fixed options.
 * Charge path must look up this table. Client-supplied amounts are ignored.
 */
...
export function lookupPromoteCatalogOption(
  ...
  return (
    PROMOTE_CATALOG.find(
      (row) => row.goal === goal && matchDuration(row, input),
    ) ?? null
  );
}
