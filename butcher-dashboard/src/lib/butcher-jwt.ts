...
/**
 * Verify a butcher-dashboard access cookie.
 * Sequence: structure → HS256 signature (algorithm not taken from header) →
 * header.alg must be HS256 → exp + role + userId.
 */
export async function verifyButcherAccessToken(
  token: string | undefined,
  secret: string | undefined,
): Promise<ButcherJwtResult> {
...
export async function signButcherAccessToken(params: {
  secret: string;
  role: string;
  userId?: string;
  expiresInSec?: number;
  ...
  const payload = bytesToBase64Url(
    new TextEncoder().encode(
      JSON.stringify({
        userId: params.userId ?? 'butcher-1',
        username: 'butcher',
        role: params.role,
        passwordVersion: 0,
        iat: now,
